...
//for verifying admin and user
int verify(); 
int chkacc();

COORD coord = {0, 0};
int m,r;
//m in main and r in record
...
//SetConsoleCursorPosition as windowAPI function 
//Retrive the handle for standard output console(GetStdHandle(STD_OUTPUT_HANDLE))
...
void welcomeScreen() {
    ...
    if (m != 1 && m != 2) {
        system("cls");
    // if number other then 1 or 2 return main function
        main();
    }
    system("CLS");
    //library is <stdlib.h>
...
int verify() {
    char a;
    if (m == 1) {
        if ((strcmp(id, "ab") == 0 && strcmp(password, "ab") == 0)) {
            gotoxy(38, 16);
            printf("You Have Successfully Logged In : \" %s \" ", id);
            gotoxy(44, 20);
            printf("Press any key to continue .... ");
            getch();
            //used for read a single character from console without displaying it.
            return 1;
        } 
        else
            return 0;
